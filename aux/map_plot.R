...
map_plot<-function(filename,var,colorchoice){
  ...
  filedata2<-data.frame(Longitude = coordinates(filename)[,1],
                        Latitude = coordinates(filename)[,2])
  ...
  if (var == "medLong"){  
    ...
    figmap <- ggplot() + geom_point(data=filedata, aes(x=Longitude, y=Latitude, colour=factor(cat)),shape=15,size=pointsize,na.rm=T) +
      scale_colour_manual(values=colorchoice,na.value = "grey50",name  ="Median\nLongevity")
